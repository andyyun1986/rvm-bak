...
%
function [PARAMETER] = SparseBayes(BASIS, Targets)
...
% 
[BASIS, BasisScales, Alpha, beta, Mu, PHI, Used] = ...
...
[SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,BASIS_B_PHI,beta] = ...
      SB2_FullStatistics(BASIS,PHI,Targets,Used, ...
			 Alpha,beta,BASIS_PHI,BASIS_Targets); 
  ...
  if (selectedAction == ACTION_TERMINATE || ...
    ...
    if abs(deltaLogBeta) > CONTROLS.MinDeltaLogBeta
      ...
      [SIGMA,Mu,S_in,Q_in,S_out,Q_out,Factor,logML,Gamma,BASIS_B_PHI] = ...
		  SB2_FullStatistics(BASIS,PHI,Targets,Used,Alpha,beta,...
							  BASIS_PHI,BASIS_Targets); 
